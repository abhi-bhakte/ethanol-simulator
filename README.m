...
% ~~~~~~~~~~~~~~~~~~~~~
%   (Note: Some files may be generated during runs and may not exist initially.)
%   • alarm_timing.txt                - Real-time alarm timing data (generated)
%   • Mouse_click.txt                 - Mouse interaction log (generated)
%   • feedback_for_tasks.txt          - Task-specific feedback responses (optional)
%   • Introduction.txt                - Deprecated/not currently used; may be generated by future runs
%   • task_no_2.txt, task_no_3.txt, task_no_5.txt - Task-specific data files (optional/generated)
%
% Excel Data Outputs:
% ~~~~~~~~~~~~~~~~~~
%   The simulator saves Excel outputs under data\excel-outputs\ with filenames built from participant ID and a time tag:
%   • Alarm_timing_<id_num>_<ty>.xlsx         - Alarm response timing analysis
%   • Mouse_click_<id_num>_<ty>.xlsx          - Click locations and timing (if enabled)
%   • Mouse_move_<id_num>_<ty>.xlsx           - Mouse movement trajectories (if enabled)
%   • Process_data_<id_num>_<ty>.xlsx         - Complete process variable history (if enabled)
%
% Notes:
%   • <id_num> is the participant or session identifier used in the GUI.
%   • <ty> is a time-based string generated at save time (e.g., 09-Feb-2026_10_35_22).
%   • Some files are only created when their respective logging modules are enabled.
%
% MATLAB Data Files:
% ~~~~~~~~~~~~~~~~~
%   (Optional/example files; may not be present in every setup)
...
% Multimedia Files:
% ~~~~~~~~~~~~~~~~
%   (Optional assets; ensure paths exist if enabled)
%   • Lower_alarm.wav, scenario_completed_audio.mp3, scenario_completed_message.wav
%   • Final_video_cognitive.mp4
%   • Thanks.jpg, image.gif
%   • reflux_added_third_pos.jpg
%
% Eye Tracking Data:
% ~~~~~~~~~~~~~~~~~
%   • gazedata/                       - Directory created when eye tracking is enabled
%   • Historical session folders may not be present by default
%
...
%
% Python API (Fault Prediction Service):
% ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
%   • Python 3.11 (64-bit)
%   • Virtual environment recommended
%   • FastAPI server in folder: explaination_module\api_server.py
%   • Simplified Python dependencies (requirements.txt):
%       absl-py, fastapi, keras, numpy, pandas, scikit-learn,
%       tensorflow, uvicorn, requests, joblib, h5py, protobuf
%
% Setup Instructions (Python):
% ~~~~~~~~~~~~~~~~~~~~~~~~~~~~
%   1. Open PowerShell and navigate to explaination_module
%      → cd "C:\Users\<user>\Desktop\Abhijit XAI Work\ethanol-simulator\explaination_module"
%   2. Create and activate venv with Python 3.11
%      → py -3.11 -m venv venv
...
%      API listens on 127.0.0.1:5000
%
% Model Artifacts:
% ~~~~~~~~~~~~~~~~
%   • Located in explaination_module\model_artifacts\
%   • scaler.joblib, label_map.joblib, baseline.joblib (required)
%   • fault_fnn.keras (Keras model)
%     - If using Keras 3, load via keras.models.load_model
%     - If using TF/Keras 2.x, provide an HDF5 (.h5) model instead
%
% Windows Notes:
% ~~~~~~~~~~~~~
%   • Enable long file paths to avoid install errors (Administrator PowerShell):
...
%
% Fault Prediction API (Python):
% ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
%   • Health check:  GET http://127.0.0.1:5000/health
%   • Predict:       POST http://127.0.0.1:5000/predict
%     Body JSON: { "features": [F101, F102, T101, T102, F105, T106, T105, T104, T103, C101, L101] }
%   • Response includes fault label, probabilities, confidence, and top features
%
% Data Analysis:
% ~~~~~~~~~~~~~
