...
global sequence_task es_flag task_complete_flag ty
global no_of_tasks Calib pts trackerId leftEyeAll rightEyeAll timeStampAll 
global task_no_lo fault_no_list_lo fault_no_lo f_ref f2_ref
...
closeButton = uicontrol(f,'Style','pushbutton','String','Submit & Close','backgroundcolor',[0.7 0.7 0.7],'fontweight','bold','fontsize',10,...
    'Units','points','Position',[1150+120 130-13+150 100 20],'Callback', @myCloseFcn, 'Visible','on');
...
set(text_for_start,'String','Press Start button to run plant','foregroundcolor',[0 0 0],'Units','points','position',...
    [450+120 20 250 30],'visible','on','fontweight','bold');
...
%           tetio_stopTracking;% please delete this after testing
...
%------------------writing ti mouse click---------------------------------
        ...
        if ~isempty(a_c) && ~isempty(b_c) && ~isempty(c_c) && ~isempty(d_c) && ~isempty(e_c) && ~isempty(f_c) && ~isempty(g_c)
            eval(sprintf('xlswrite(''data\\excel-outputs\\Mouse_click_%s.xlsx'',[a_c b_c c_c d_c e_c f_c g_c],%d);',ty,task_no));
        end
...
%------------------- alarm data --------------------
        ...
        if ~isempty(a_a) && ~isempty(b_a) && ~isempty(c_a) 
            eval(sprintf('xlswrite(''data\\excel-outputs\\Alarm_timing_%s.xlsx'',[a_a b_a c_a],%d);',ty,task_no));
...
%%--------------------------writing to process data-----------------------
        ...
        eval(sprintf('xlswrite(''data\\excel-outputs\\Process_data_%s.xlsx'',process_var_store,%d);',ty,task_no));
        ...
        if ~isempty(a_c) && ~isempty(b_c) && ~isempty(c_c) && ~isempty(d_c) && ~isempty(e_c) && ~isempty(f_c)
            eval(sprintf('xlswrite(''data\\excel-outputs\\Mouse_move_%s.xlsx'',[a_c b_c c_c d_c e_c f_c],%d);',ty,task_no));
        end
