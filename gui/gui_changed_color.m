...
% Current fault prediction text (summary)
fault_prediction_axes = uicontrol(trendPanelRight,'Style','text', ...
    'String', '', ...
    'Units','normalized','Position',[0.03 0.86 0.94 0.05], ...
    'BackgroundColor',.9.*[1 1 1],'ForegroundColor',[0.35 0.35 0.35], ...
    'HorizontalAlignment','left','FontSize',10,'FontWeight','bold', ...
    'Visible','off');

fault_prediction_text = uicontrol(trendPanelRight,'Style','text', ...
    'String', '', ...
    'Units','normalized','Position',[0.03 0.64 0.94 0.19], ...
    'BackgroundColor',.9.*[1 1 1],'ForegroundColor',[0 0 0], ...
    'HorizontalAlignment','left','FontSize',9,'FontWeight','bold');

% Effects label and text
fault_consequence_label = uicontrol(trendPanelRight,'Style','text', ...
    'String', 'Effects:', ...
    'Units','normalized','Position',[0.03 0.53 0.3 0.06], ...
    'BackgroundColor',.9.*[1 1 1],'ForegroundColor',[0 0 0], ...
    'HorizontalAlignment','left','FontSize',10,'FontWeight','bold', ...
    'Visible','off');

fault_consequence_text = uicontrol(trendPanelRight,'Style','text', ...
    'String', '', ...
    'Units','normalized','Position',[0.03 0.35 0.94 0.16], ...
    'BackgroundColor',.9.*[1 1 1],'ForegroundColor',[0.35 0.35 0.35], ...
    'HorizontalAlignment','left','FontSize',10,'FontWeight','normal', ...
    'Visible','off');

% Corrective action label and text
fault_action_label = uicontrol(trendPanelRight,'Style','text', ...
    'String', 'Corrective Action:', ...
    'Units','normalized','Position',[0.03 0.24 0.35 0.06], ...
    'BackgroundColor',.9.*[1 1 1],'ForegroundColor',[0 0 0], ...
    'HorizontalAlignment','left','FontSize',10,'FontWeight','bold', ...
    'Visible','off');

fault_action_text = uicontrol(trendPanelRight,'Style','text', ...
    'String', '', ...
    'Units','normalized','Position',[0.03 0.06 0.94 0.14], ...
    'BackgroundColor',.9.*[1 1 1],'ForegroundColor',[0.35 0.35 0.35], ...
    'HorizontalAlignment','left','FontSize',10,'FontWeight','normal', ...
    'Visible','off');
...
function v16Fcn(hObject,eventdata,handles)
        ...
        % After calibration, show Task Introduction (not the plant UI yet)
        try, close(f1); catch, end
        try, close(f); catch, end
        try, close(f2); catch, end
        making_ready_for(task_no_lo,fault_no_list_lo,fault_no_lo);
    end
